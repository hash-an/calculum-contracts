    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        traderBotWallet = _traderBotWallet;
    ...
    function NextEpoch() internal returns (uint256) {
        if (
            block.timestamp >
            EPOCH_START.add(EPOCH_DURATION.mul(CURRENT_EPOCH.add(1)))
        ) {
            ++CURRENT_EPOCH;
        }
        ...
        returns (uint256)
    {
        ...
        if (_assets > (maxDeposit(_receiver).sub(depositor.finalAmount.add(depositor.amountAssets))))
        ...
        returns (uint256 _assets)
    {
        ...
        if (CURRENT_EPOCH == 0) return (supply == 0)
    ...
    {
		uint256 supply = totalSupply();
        if (CURRENT_EPOCH == 0) return (_assets == 0 || supply == 0)
    ...
    function setInitialValue(uint256[3] memory _initialValue)
        external
        onlyOwner
    {
